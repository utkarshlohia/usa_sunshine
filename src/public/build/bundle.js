...
var app = (function () {
    ...
    // (236:12) {#each Array.from({length: 11}, (_, i) => 2010 + i) as year}
    function create_each_block(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			add_location(option, file, 236, 16, 7850);
    		},
    	...
    	dispatch_dev("SvelteRegisterBlock", {
    		...
    		type: "each",
    		source: "(236:12) {#each Array.from({length: 11}, (_, i) => 2010 + i) as year}",
    		ctx
    	});
    ...
    function create_fragment(ctx) {
    	...
    	let t3;
    	let span;
    	let t4_value = /*months*/ ctx[3][/*selectedMonth*/ ctx[1]] + "";
    	let t4;
    	let t5;
    	let t6;
    	let t7;
    	...
    	let mounted;
    	let dispose;
    	let each_value = Array.from({ length: 11 }, func);
    	...
    	const block = {
    		c: function create() {
    			main = element("main");
    			h1 = element("h1");
    			h1.textContent = "Minutes of Sunshine in the United States 2010-2020";
    			t1 = space();
    			div0 = element("div");
    			...
    			t4 = text$1(t4_value);
    			t5 = space();
    			t6 = text$1(/*selectedYear*/ ctx[0]);
    			t7 = space();
    			div1 = element("div");
    			svg = svg_element("svg");
    			attr_dev(h1, "color", "darkgray");
    			attr_dev(h1, "class", "svelte-1vega14");
    			add_location(h1, file, 232, 4, 7475);
    			if (/*selectedYear*/ ctx[0] === void 0) add_render_callback(() => /*select_change_handler*/ ctx[6].call(select));
    			add_location(select, file, 234, 8, 7586);
    			attr_dev(input, "id", "date-slider");
    			attr_dev(input, "type", "range");
    			attr_dev(input, "min", "0");
    			attr_dev(input, "max", "11");
    			input.value = /*selectedMonth*/ ctx[1];
    			attr_dev(input, "class", "svelte-1vega14");
    			add_location(input, file, 240, 8, 7934);
    			attr_dev(span, "id", "date-display");
    			attr_dev(span, "class", "svelte-1vega14");
    			add_location(span, file, 241, 8, 8049);
    			attr_dev(div0, "id", "controls");
    			attr_dev(div0, "class", "svelte-1vega14");
    			add_location(div0, file, 233, 4, 7558);
    			attr_dev(main, "class", "svelte-1vega14");
    			add_location(main, file, 231, 0, 7464);
    			attr_dev(svg, "id", "map");
    			attr_dev(svg, "class", "svelte-1vega14");
    			add_location(svg, file, 246, 4, 8168);
    			attr_dev(div1, "id", "map-container");
    			attr_dev(div1, "class", "svelte-1vega14");
    			add_location(div1, file, 245, 0, 8139);
    		},
    		...
    		m: function mount(target, anchor) {
    			...
    			select_option(select, /*selectedYear*/ ctx[0], true);
    			append_dev(div0, t2);
    			...
    			if (!mounted) {
    				dispose = [
    					...
    					listen_dev(select, "change", /*change_handler*/ ctx[7], false, false, false, false),
    					listen_dev(input, "input", /*handleSliderChange*/ ctx[5], false, false, false, false)
    				];
    		...
    		p: function update(ctx, [dirty]) {
    			if (dirty & /*Array*/ 0) {
    				each_value = Array.from({ length: 11 }, func);
    				validate_each_argument(each_value);
    				let i;
    			...
    			if (dirty & /*selectedYear, Array*/ 1) {
    				select_option(select, /*selectedYear*/ ctx[0]);
    			}

    			if (dirty & /*selectedMonth*/ 2) {
    				prop_dev(input, "value", /*selectedMonth*/ ctx[1]);
    			}

    			if (dirty & /*selectedMonth*/ 2 && t4_value !== (t4_value = /*months*/ ctx[3][/*selectedMonth*/ ctx[1]] + "")) set_data_dev(t4, t4_value);
    			if (dirty & /*selectedYear*/ 1) set_data_dev(t6, /*selectedYear*/ ctx[0]);
    		},
    		i: noop$4,
    		o: noop$4,
    		d: function destroy(detaching) {
    ...
    function instance($$self, $$props, $$invalidate) {
    	...
    	let colorScale = sequential(OrRd);

    	// Use Albers USA projection to include Alaska and Hawaii
    	let projection = albersUsa().scale(1400).translate([width / 2, height / 2]); // Adjust the scale for better fit

    	const pathGenerator = index$2().projection(projection);

    	onMount(async () => {
    		try {
    			data = await csv('proj3_us_df.csv', d => {
    				return {
    					...d,
    	...
    	function drawMap() {
    		...
    		svg.selectAll('path').data(geojsonData.features).enter().append('path').attr('d', pathGenerator).attr('fill', 'lightgray'); // Default color before data is processed
    	...
    	function updateMap(date) {
    		...
    		const points = svg.selectAll('.point').data(filteredData, d => d.station_id);

    		const pointsEnter = points.enter().append('circle').attr('class', 'point').attr('cx', d => {
    			const coords = projection([d.longitude, d.latitude]);
    			return coords ? coords[0] : -9999;
    		}).attr('cy', d => {
    			const coords = projection([d.longitude, d.latitude]);
    			return coords ? coords[1] : -9999;
    		}).attr('r', 0).attr('fill', 'rgba(255, 255, 153, 0.7)').on('mouseover', function (event, d) {
    			select(this).attr('r', 3 + d.sunshine_total_min / 50).attr('fill', 'orange'); // Initial radius for smooth transition
    			svg.append('text').attr('id', 'tooltip').attr('x', projection([d.longitude, d.latitude])[0] + 10).attr('y', projection([d.longitude, d.latitude])[1] - 10).attr('fill', 'black').text(`Sunshine: ${format('.2f')(d.sunshine_total_min)} mins`);
    		}).on('mouseout', function (event, d) {
    			select(this).attr('r', 2 + d.sunshine_total_min / 50).attr('fill', 'rgba(255, 255, 153, 0.7)'); // Revert to lighter yellow
    			svg.select('#tooltip').remove();
    		});

    		pointsEnter.merge(points).transition().duration(800).attr('r', d => 2 + d.sunshine_total_min / 50).attr('fill', 'rgba(255, 255, 153, 0.7)');
    		points.exit().transition().duration(500).attr('r', 0).remove(); // Smooth exit
    		...
    		filteredData.forEach(d => {
    			...
    			for (let i = 0; i < 8; i++) {
    				...
    				svg.append('line').attr('class', 'ray').attr('x1', coords[0]).attr('y1', coords[1]).attr('x2', coords[0]).attr('y2', coords[1]).attr('stroke', rayColor).attr('stroke-width', 1.5).transition().duration(500).attr('x2', coords[0] + rayLength * Math.cos(angle)).attr('y2', coords[1] + rayLength * Math.sin(angle));
    			}
    		});
    	}

    	function handleSliderChange(event) {
    		$$invalidate(1, selectedMonth = event.target.value);
    		$$invalidate(2, selectedDate = `${selectedYear}-${String(parseInt(selectedMonth) + 1).padStart(2, '0')}-01`);
